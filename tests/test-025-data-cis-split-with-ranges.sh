...
for chunk in `ls ${TMPD}/chunks` ; do
  ...
  ../import-pairs \
      <(../cis-ranges-to-pairs <(cut -f 1 ${TMPD}/chunks/${chunk}/genotype.fat | sed 1d) < ${TMPD}/chunks/${chunk}/cisRanges.tab 2> /dev/null) \
      ${TMPD}/chunks/${chunk}/cisPairs.sqlite \
      2> /dev/null \
      > /dev/null

  ../eqtl-ranef \
    --regressors=<(cat ${TMPD}/chunks/${chunk}/genotype.fat | ../table-melt 2> /dev/null) \
    --outcomes=<(cat ${TMPD}/chunks/${chunk}/expression.fat | ../table-melt 2> /dev/null) \
    --fixef=${FIXEF} \
    --ranef=${RANEF} \
